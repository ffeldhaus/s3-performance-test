...
public class PerformanceTest {
    ...
    private static File createSampleFile(final long sizeInMb, final String tempFileDirectory) throws IOException {
        ...
        if (StringUtils.isNullOrEmpty(tempFileDirectory)) {
            ...
            file = File.createTempFile("aws-java-sdk-", ".txt", directory);
        }
        file.deleteOnExit();

        OutputStream outputStream = new FileOutputStream(file);
        InputStream inputStream = new RandomStream(sizeInMb * MB, RandomUtils.nextInt());
    ...
    /**
     * Creates a temporary inputStream of size sizeInMb with only zeroes to demonstrate uploading to Amazon S3
     *
     * @return A newly created inputStream with zeroes of size siteInMb.
     * @throws IOException
     */
    ...
    private class MultiHttpClientConnThread extends Thread {
        ...
        public MultiHttpClientConnThread(final CloseableHttpClient client, final HttpGet get, final PoolingHttpClientConnectionManager connManager, final long startByte) throws java.io.IOException {
            this.file = File.createTempFile("aws-java-sdk-", ".txt");
            this.client = client;
            this.get = get;
            this.connManager = connManager;
            this.startByte = startByte;
        ...
        public final void run() {
            try {
                ...
                final int size = 4 * (int)KB;
                long offset = this.startByte;
                for (final byte[] buffer = new byte[size]; content.read(buffer) != -1;) {
                    raf.seek(offset);
                    raf.write(buffer);
                    offset += size;
                }
    ...
    private void run() {
        ...
        final int partSize = (sizeInMb / numberOfProcessors) > 512 ? (512 * MB) : (sizeInMb * MB / numberOfProcessors);
        ...
        try {
            logger.info("Creating temporary file of size " + sizeInMb + "MB. This may take a while...");
            ...
            */
            ...
            for (long partNumber = 1; partNumber <= numberOfParts; partNumber++) {
                InputStream sampleInputStream = createSampleInputStream(partSize / MB);
